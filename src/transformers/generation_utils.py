...
@dataclass
class GreedySearchEncoderDecoderOutputCustom(ModelOutput):
    ...
    cross_attentions: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    decoder_hidden_states: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    possible_next_wrods: torch.LongTensor = None
    possible_next_wrods_scores: torch.LongTensor = None
    custom_model_kwargs: Optional[dict] = None
    cur_len: Optional[int] = None
    custom_logit_processor: Optional[LogitsProcessorList] = None
    custom_stopping_criteria: Optional[StoppingCriteriaList] = None
...
@dataclass
class BeamSearchEncoderDecoderOutputSuggestion(ModelOutput):
    ...
    custom_model_kwargs: Optional[dict] = None
    cur_len: Optional[int] = None
    custom_beam_scorer: Optional[BeamScorer] = None
    custom_logit_processor: Optional[LogitsProcessorList] = None
    custom_stopping_criteria: Optional[StoppingCriteriaList] = None
...
GreedySearchOutput = Union[GreedySearchEncoderDecoderOutput, GreedySearchDecoderOnlyOutput, GreedySearchEncoderDecoderOutputCustom]
SampleOutput = Union[SampleEncoderDecoderOutput, SampleDecoderOnlyOutput]
BeamSearchOutput = Union[BeamSearchEncoderDecoderOutput, BeamSearchDecoderOnlyOutput,BeamSearchEncoderDecoderOutputSuggestion]
...
class GenerationMixin:
    ...
    @torch.no_grad()
    def generate_with_suggestion(
            self,
            input_ids: Optional[torch.LongTensor] = None,
            output_ids: Optional[List] = None,
            custom_model_args: Optional[dict] = None,
            current_length: Optional[int] = None,
            custom_beam_scorer: Optional[BeamScorer] = None,
            custom_scores: Optional[Tuple[torch.FloatTensor]] = None,
            custom_beam_scores: Optional[torch.FloatTensor] = None,

            custom_logit_processor: Optional[LogitsProcessorList] =None,
            custom_encoder_attentions: Optional[Tuple[torch.FloatTensor]] = None,
            custom_encoder_hidden_states: Optional[Tuple[torch.FloatTensor]] = None,
            custom_decoder_attentions: Optional[Tuple[Tuple[torch.FloatTensor]]] = None,
            custom_cross_attentions: Optional[Tuple[Tuple[torch.FloatTensor]]] = None,
            custom_decoder_hidden_states: Optional[Tuple[Tuple[torch.FloatTensor]]] = None,
            custom_stopping_criteria: Optional[StoppingCriteriaList] = None,

            num_of_next_word_suggestions_list: Optional[int] = None,
            ...
            synced_gpus: Optional[bool] = None,
            **model_kwargs,
    ) -> Union[GreedySearchOutput,BeamSearchOutput, torch.LongTensor]:


        if output_ids is not None and custom_model_args is not None:
            if num_beams==1:
                output_ids_tensor=torch.tensor(np.array([output_ids]))

                # greedy search
                return self.greedy_search_custom(
                    input_ids,
                    logits_processor=custom_logit_processor,
                    ...
                    return_dict_in_generate=return_dict_in_generate,
                    synced_gpus=synced_gpus,
                    cur_len=current_length,
                    custom_model_args=custom_model_args,
                    output_ids=output_ids_tensor,
                    scores=custom_scores,
                    encoder_attentions=custom_encoder_attentions,
                    encoder_hidden_states=custom_encoder_hidden_states,
                    decoder_attentions=custom_decoder_attentions,
                    cross_attentions=custom_cross_attentions,
                    decoder_hidden_states=custom_decoder_hidden_states,
                    num_of_next_word_suggestions_list=num_of_next_word_suggestions_list,
                    **model_kwargs,
                )

            elif num_beams>1:

                output_ids_tensor=torch.tensor(np.array([output_ids, output_ids, output_ids]))

                return self.beam_search_custom(
                    input_ids,
                    custom_beam_scorer,
                    logits_processor=custom_logit_processor,
                    stopping_criteria=custom_stopping_criteria,
                    ...
                    output_ids=output_ids_tensor,
                    scores=custom_scores,
                    beam_scores=custom_beam_scores,
                    encoder_attentions=custom_encoder_attentions,
                    encoder_hidden_states=custom_encoder_hidden_states,
                    decoder_attentions=custom_decoder_attentions,
                    cross_attentions=custom_cross_attentions,
    ...
    @torch.no_grad()
    def generate_with_suggestion_completion(
            self,
            input_ids: Optional[torch.LongTensor] = None,
            output_ids: Optional[List] = None,
            max_length: Optional[int] = None,
        ...
        if is_greedy_gen_mode:
            ...
            if output_ids is not None:
                input_ids = torch.tensor(np.array([output_ids]))
            # interleave with `num_beams`
            input_ids, model_kwargs = self._expand_inputs_for_generation(
                input_ids, expand_size=num_beams, is_encoder_decoder=self.config.is_encoder_decoder, **model_kwargs
            )
            return self.beam_search_suggestion_completion(
                input_ids,
                beam_scorer,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
    ...
    def beam_search_suggestion_completion(
            self,
            input_ids: torch.LongTensor,
        ...
        while True:
            ...
            next_token_scores = next_token_scores.view(batch_size,
                                                       num_beams * vocab_size)  # tensor([[-16.8717, -16.9149, -13.9981,  ..., -24.0156, -28.2072, -19.7235]])
            ...
            if count == 1:
                possible_next_wrods=next_tokens
        ...
        if return_dict_in_generate:
            ...
            if self.config.is_encoder_decoder:
                return BeamSearchEncoderDecoderOutputSuggestion(
                    ...
                    possible_next_wrods=possible_next_wrods,
                )
            else:
    ...
    def greedy_search_custom(
        ...
        num_of_next_word_suggestions_list: Optional[int] = 5,
        **model_kwargs,
        ...
        while x==1:
            ...
            if custom_model_args is not None:
                model_kwargs=custom_model_args
            if output_ids is not None:
            ...
            next_token_scores_list, next_tokens_list = torch.topk(
                # Returns the k largest elements of the given input tensor along a given dimension.
                next_token_scores, num_of_next_word_suggestions_list, dim=1, largest=True, sorted=True
            )
            ...
            #     next_tokens = next_tokens[0] * unfinished_sequences + pad_token_id * (1 - unfinished_sequences)
        ...
        if return_dict_in_generate:
            if self.config.is_encoder_decoder:
                return GreedySearchEncoderDecoderOutputCustom(
                    sequences=input_ids,
                    scores=scores,
                    encoder_attentions=encoder_attentions,
                    encoder_hidden_states=encoder_hidden_states,
                    ...
                    cross_attentions=cross_attentions,
                    decoder_hidden_states=decoder_hidden_states,
                    possible_next_wrods=next_tokens_list,
                    possible_next_wrods_scores=next_token_scores_list,
                    custom_model_kwargs=model_kwargs,
                    cur_len=cur_len,
                    custom_logit_processor=logits_processor,
                    custom_stopping_criteria=stopping_criteria,
                )
            else:
    ...
    def beam_search_custom(
        ...
        **model_kwargs,
        ...
        while (x==1):
            ...
            if custom_model_args is not None and output_ids is not None:
                model_kwargs=custom_model_args
                input_ids=output_ids
                model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            else:
        ...
        if return_dict_in_generate:
            ...
            if self.config.is_encoder_decoder:
                return BeamSearchEncoderDecoderOutputSuggestion(
                    ...
                    cross_attentions=cross_attentions,
                    decoder_hidden_states=decoder_hidden_states,
                    possible_next_wrods=beam_outputs["next_beam_tokens"],
                    custom_model_kwargs=model_kwargs,
                    cur_len=cur_len,
                    custom_beam_scorer=beam_scorer,
                    custom_logit_processor=logits_processor,
                    custom_stopping_criteria=stopping_criteria,
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            print("beam_next_tokens:{}:{}:{}:{}".format(beam_next_tokens, x_beam_next_tokens, beam_scores, beam_idx))
            ...
            for seq in input_ids:
                print("partial seq: {}".format(tokenizer.batch_decode(seq, skip_special_tokens=True)))
